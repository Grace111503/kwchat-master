...
public interface MessageMapper extends BaseMapper<Message> {
     ...
     * 查询会话消息列表（分页）
     */
    @Select("SELECT * FROM chat_message WHERE conversation_id = #{conversationId} AND deleted = 0 " +
     ...
     * 查询消息详情
     */
    @Select("SELECT * FROM chat_message WHERE id = #{id} AND deleted = 0")
    Message selectById(@Param("id") Long id);

    /**
     * 根据客户端消息ID查询
     */
    @Select("SELECT * FROM chat_message WHERE client_message_id = #{clientMessageId} AND deleted = 0")
    Message selectByClientMessageId(@Param("clientMessageId") String clientMessageId);

    /**
     * 查询会话中未读的消息（排除自己发送的）
     */
    @Select("SELECT * FROM chat_message WHERE conversation_id = #{conversationId} AND sender_id != #{userId} AND is_read = 0 AND deleted = 0 " +
